...
router.get('/', eAdmin, async (req, res) => {
    ...
    if (1 !== 0) {
      ...
      escalas.forEach(escala => {
        ...
        return res.render("admin/consultarEscalas/list", { layout: 'main', profile: req.user.dataValues, sidebarSituations: true, escalasAgrupadas: escalasAgrupadas, danger_msg: 'Nenhuma Escala Encontrada' });
        ...
        return res.render("admin/consultarEscalas/list", { layout: 'main', profile: req.user.dataValues, sidebarSituations: true, danger_msg: 'Erro: Nenhum omes encontrada!' });
    }
  ...
  try {
    ...
    for (const { idevento, data_inicio, hora_inicio, hora_fim } of combinacoes) {
      const escalasAssociados = await db.escalas.findAll({
        attributes: ['id', 'pg', 'matricula', 'nome', 'telefone','ome_sgpm', 'modalidade' , 'hora_inicio', 'hora_fim', 'anotacoes'],
        where: {
          idevento: idevento,
